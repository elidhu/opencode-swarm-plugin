/**
 * OpenCode Hive Plugin Wrapper
 *
 * This is a thin wrapper that shells out to the `hive` CLI for all tool execution.
 * Generated by: hive setup
 *
...
/**
 * Execute a hive tool via CLI
 *
 * Spawns `hive tool <name> --json '<args>'` and returns the result.
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("error", (err) => {
      if ((err as NodeJS.ErrnoException).code === "ENOENT") {
        reject(
          new Error(
            `hive CLI not found. Install with: npm install -g opencode-hive-plugin`,
          ),
        );
      } else {
        reject(err);
...
// =============================================================================
// Hive Mail Tools (Embedded)
// =============================================================================

const hivemail_init = tool({
  description: "Initialize Hive Mail session (REQUIRED FIRST)",
  args: {
...
const hivemail_send = tool({
  description: "Send message to other agents via Hive Mail",
  args: {
    to: tool.schema
...
const hivemail_health = tool({
  description: "Check Hive Mail database health",
  args: {},
...
const hive_init = tool({
  description: "Initialize hive session and check tool availability",
  args: {
